/**
 ...
 07/31/2015
 */
define(["durandal/app", "durandal/viewlocator", "knockout", "viewmodels/header", "q"], function (app, viewLocator, ko, Header, Q) {
    return function () {
        var me = this;
        ...
        var buildingViewTreeForNode = function (nodeConfig) {
            ...
            if (viewsetName) {
                ...
                require(["viewmodels/" + viewsetName], function (cls) {
                    var viewset = new cls;
                    for (var i = 0; i < items.length; i++)if (items[i].type === "view")queue.push(createView(items[i])); else if (items[i].type === "viewset")queue.push(buildingViewTreeForNode(items[i]));
                    Q.all(queue).then(function (views) {
                        for (var index in views)viewset[items[index].location](views[index]);
                        defer.resolve(viewset)
                    })
                })
            } else if (viewName)createView(nodeConfig).done(function (cls) {
                defer.resolve(cls)
            });
            return defer.promise
        };
