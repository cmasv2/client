define(['durandal/app', 'plugins/router', "knockout","gauges","./../bindings/gauges", "./../bindings/status-binding"],
function (app, router, ko) {
    return function (cf) {
        var channels = [],me = this,buff = app.buff,Soc = app.services.getService("socket");
        function pr(it, col) {
            var r;for (var i = 0, j = it.length; i < j; i++){if (i % col === 0){if (r)channels.push(r);r = [];}r.push(it[i]);}if (r)channels.push(r);
        }
        function Lis(c) {
        ...
        for (var i = 0; i < cf[0].channels.length; i++) {
            ...
            it[0]["value1"].unit = info?info[0].unit:"NaN";
            Lis(it[0]["value1"]);
            if (it[1]) {
                binds(it[1].status);
                ...
                it[1]["value1"].unit = info?info[0].unit:"NaN";
                Lis(it[1]["value1"]);
            }
        }
        me.viewdetail = function(c){c.code!=undefined?router.navigate("page/" + c.url + "/" + c.code):router.navigate("page/detail/ITW_PW");};
