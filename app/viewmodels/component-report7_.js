/**
 ...
 07/31/2015
 */
define(['durandal/app', "knockout", "knockback", 'plugins/router','q', "../models/report","moment","jqplot","cursor","dateAxisRenderer","canvasTextRenderer","canvasAxisLabelRenderer", "../bindings/datetime.picker", "bootstrap-datepicker","css!../../lib/nqhlib/css/jquery.jqplot.min.css"],
    function (app, ko, kb, router,Q ,Report,Moment) {
        return function () {
            var me = this;
            ...
            function getHistories(){
                ...
                apis.averageByTimeRange(buff.find(buff.getByKey('ChannelsAll'), 'code', 'DPMA_kWh')[0].id, fromTime, toTime).then(function(rs){
                    if(rs.result){
                        me.DPMA_kWh(Math.round(rs.result * 10) / 10);
                    }
                ...
                apis.getHistories1(idtemp, fromTime, toTime,1000).then(function (r){
                    ...
                    apis.averageByTimeRange(idtemp, fromTime, toTime).then(function(rs){
                        if(rs.result){
                            me.Temp(Math.round(rs.result * 10) / 10);
                        }
                    });
                    if (response != undefined){
                        for (var i in response) {
                            me.Temps.push([Moment(response[i]["createdAt"]).format('MM/DD/YYYY hh:mm:ss'), response[i]["value"]]);
                        }
                    }
                    var idhum = buff.find(buff.getByKey('ChannelsAll'), 'code', 'ENV_ServerHum1')[0].id;
                    apis.getHistories1(idhum, fromTime, toTime,1000).then(function (r){
                        response = r.result.sort(function(a,b) { return a.createdAt < b.createdAt ? 1 : a.createdAt > b.createdAt ?  -1 : 0; });
                        apis.averageByTimeRange(idhum, fromTime, toTime).then(function(rs){
                            if(rs.result){
                                me.Hum(Math.round(rs.result * 10) / 10);
                            }
                        });
                        if (response != undefined){
                            for (var i in response) {
                                me.Hums.push([Moment(response[i]["createdAt"]).format('MM/DD/YYYY hh:mm:ss'), response[i]["value"]]);
                            }
                        }
                        deferred.resolve();
                    });
