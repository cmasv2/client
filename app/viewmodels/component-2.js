...
define(["durandal/app", "plugins/router", "knockout", "./../bindings/status-binding"], function (app, router, ko) {
    return function (cf) {
        ...
        function Lis(c) {
            ...
            var cv = function (value) {
                if (c.type != undefined) {
                    ...
                    if (c.type == "3")if (value != undefined)c.value(c.text1);
                    if (c.type == "4") {
                        var t = value % 3600;
                        var h = (value - t) / 3600;
                        var m = (t - t % 60) / 60;
                ...
                } else if (value != undefined)c.value(number_format(Math.round(value * 10) / 10))
            };
        ...
        function number_format(number, decimals, dec_point, thousands_sep) {
            ...
            if (prec >= 1 && decPos !== -1 && s.length - decPos - 1 < prec)s += (new Array(prec - (s.length - decPos - 1))).join(0) + "0"; else if (prec >= 1 && decPos === -1)s += dec + (new Array(prec)).join(0) + "0";
            return s
        }
        ...
        for (var i = 0; i < cf[0].channels.length; i++) {
            ...
            if (it["value"] != undefined)for (j = 0; j < it["value"].length; j++) {
